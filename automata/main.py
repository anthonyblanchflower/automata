...
class Launcher(tkinter.Tk):
    ...
    def initialize(self):
        ...
        self.menuVariable_Rule = tkinter.StringVar()
        self.ruleList = []
        for root, dirs, files in os.walk(os.path.dirname(os.path.realpath(sys.argv[0])) + "/data/rules"):
            for file in files:
                if file.endswith('.json'):
                    self.ruleList.append(file)
        self.menu_rule = tkinter.OptionMenu(self, self.menuVariable_Rule, *self.ruleList)
        self.menu_rule.grid(column=0, row=0, sticky='EW')
        ...
        self.menu_seed.grid(column=0, row=3, sticky='EW')
        ...
        self.menu_x.grid(column=0, row=6, sticky='EW')
        ...
        self.menu_y.grid(column=0, row=9, sticky='EW')
        ...
        label_x = tkinter.Label(self, textvariable=self.labelVariable_X,
                                anchor="w", fg="green", bg="black")
        label_x.grid(column=0, row=7, columnspan=2, sticky='EW')
        ...
        label_y = tkinter.Label(self, textvariable=self.labelVariable_Y,
                                anchor="w", fg="green", bg="black")
        label_y.grid(column=0, row=10, columnspan=2, sticky='EW')
        ...
        label_scale = tkinter.Label(self, textvariable=self.labelVariable_Scale,
                                    anchor="w", fg="green", bg="black")
        label_scale.grid(column=0, row=13, columnspan=2, sticky='EW')
    ...
    def onbuttonclick_rule(self):
        self.labelVariable_Rule.set(self.menuVariable_Rule.get())
    ...
    def onbuttonclick_launch(self):
        self.automataCall = sys.executable + " " + os.path.dirname(os.path.realpath(sys.argv[0])) + "/Automata.py -r " \
                        + self.labelVariable_Rule.get() + \
                        " -s " + self.labelVariable_Seed.get() + \
